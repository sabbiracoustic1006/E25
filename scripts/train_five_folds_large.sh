...
for O_WEIGHT in ${O_WEIGHTS}; do
  ...
  echo "=== Training DeBERTa-v3-large (lr=${LR}, o_weight=${O_WEIGHT}) for 5 folds ==="
  ...
  for fold in {0..4}; do
    ...
    CUDA_VISIBLE_DEVICES=1 python train_new_reorganized_eval_score.py \
      --model_id "${MODEL_ID}" \
    ...
    # Refine thresholds
    CUDA_VISIBLE_DEVICES=1 python inference_threshold_refine_mean_sd.py \
    ...
    # Generate submission
    CUDA_VISIBLE_DEVICES=1 python submit_quiz_with_thresholds.py \
      --model_dir "${FOLD_MODEL_DIR}" \
      --threshold_path "${OUTPUT_DIR}/refined_thresholds_fold${fold}.json" \
      --start_idx 5001 \
      --end_idx 30000 \
      --output_file "${TSV_DIR}/deberta_v3_large_lr_${LR_SLUG}_o_weight_${O_WEIGHT_SLUG}_fold${fold}.tsv"
  done
