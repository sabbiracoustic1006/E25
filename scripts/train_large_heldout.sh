...
echo "=== Training DeBERTa-v3-large with held-out set and multi-epoch evaluation ==="
echo "Model: ${MODEL_ID}"
...
for fold in {0..4}; do
  ...
  for epoch in 5 6 7 8 9; do
    ...
    CUDA_VISIBLE_DEVICES=0 python generate_submission.py \
      ...
      --start_idx 5001 \
      --end_idx 30000 \
      --output_file "${TSV_DIR}/deberta_v3_large_heldout_lr_${LR_SLUG}_o_weight_${O_WEIGHT_SLUG}_fold${fold}_epoch${epoch}.tsv"
    ...
    echo "  - Submission: ${TSV_DIR}/deberta_v3_large_heldout_lr_${LR_SLUG}_o_weight_${O_WEIGHT_SLUG}_fold${fold}_epoch${epoch}.tsv"
    echo ""
  done
...
echo "  Submissions: ${TSV_DIR}/deberta_v3_large_heldout_lr_${LR_SLUG}_o_weight_${O_WEIGHT_SLUG}_fold*_epoch*.tsv"
echo "  Held-out data: ${OUTPUT_BASE}/heldout_data/heldout_set_ratio${HELDOUT_RATIO}.tsv"
echo ""
