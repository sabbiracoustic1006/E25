...
readonly EXPERIMENT_ID="custom_ls"
...
for O_WEIGHT in ${O_WEIGHTS}; do
  ...
  OUTPUT_DIR="${DATA_ROOT}/ablation_study/${MODEL_SLUG}_${EXPERIMENT_ID}/lr_${LR_SLUG}/o_weight_${O_WEIGHT_SLUG}"
  mkdir -p "${OUTPUT_DIR}"

  echo "=== Training DeBERTa-v3-base with custom label smoothing (lr=${LR}, o_weight=${O_WEIGHT}) for 5 folds ==="

  # Train and process each fold
  for fold in {0..4}; do
    echo "=== Training fold ${fold} with o_weight=${O_WEIGHT} and custom label smoothing ==="

    # Train model for this fold with custom label smoothing
    CUDA_VISIBLE_DEVICES=2 python train_new_reorganized_eval_score.py \
      ...
      --output_dir "${DATA_ROOT}/ablation_study/${MODEL_SLUG}_${EXPERIMENT_ID}/lr_${LR_SLUG}"
    ...
    CUDA_VISIBLE_DEVICES=2 python submit_quiz_with_thresholds.py \
      ...
      --start_idx 5001 \
      --end_idx 30000 \
      --output_file "${TSV_DIR}/deberta_v3_base_${EXPERIMENT_ID}_lr_${LR_SLUG}_o_weight_${O_WEIGHT_SLUG}_fold${fold}.tsv"
  done

  echo "=== Completed all 5 folds for o_weight=${O_WEIGHT} ==="
done

echo "=== All o_weight experiments with custom label smoothing completed ==="