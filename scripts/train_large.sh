...
echo "=== Training DeBERTa-v3-large with multi-epoch checkpoint evaluation ==="
echo "Model: ${MODEL_ID}"
...
for fold in {0..4}; do
  ...
  for epoch in 5 6 7; do
    ...
    CUDA_VISIBLE_DEVICES=0 python generate_submission.py \
      ...
      --start_idx 5001 \
      --end_idx 30000 \
      --output_file "${TSV_DIR}/deberta_v3_large_lr_${LR_SLUG}_o_weight_${O_WEIGHT_SLUG}_fold${fold}_epoch${epoch}.tsv"

    echo "✓ Epoch ${epoch} completed!"
    echo "  - Thresholds: ${OUTPUT_BASE}/refined_thresholds_categorywise_fold${fold}_epoch${epoch}.json"
    echo "  - Submission: ${TSV_DIR}/deberta_v3_large_lr_${LR_SLUG}_o_weight_${O_WEIGHT_SLUG}_fold${fold}_epoch${epoch}.tsv"
    echo ""
  done
...
echo "  Submissions: ${TSV_DIR}/deberta_v3_large_lr_${LR_SLUG}_o_weight_${O_WEIGHT_SLUG}_fold*_epoch*.tsv"
echo ""
