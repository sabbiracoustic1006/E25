...
for O_WEIGHT in ${O_WEIGHTS}; do
  ...
  echo "=== Training DeBERTa-v3-small (lr=${LR}, o_weight=${O_WEIGHT}) for 5 folds ==="
  ...
  for fold in {0..4}; do
    ...
    CUDA_VISIBLE_DEVICES=0 python submit_quiz_with_thresholds.py \
      ...
      --start_idx 5001 \
      --end_idx 30000 \
      --output_file "${TSV_DIR}/deberta_v3_small_lr_${LR_SLUG}_o_weight_${O_WEIGHT_SLUG}_fold${fold}.tsv"
  done
