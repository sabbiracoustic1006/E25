...
def build_label_resources(
    ...
    """Derive label vocabularies from the provided dataframes."""
    aspects = sorted({asp for asp in base_df["Aspect Name"].unique() if asp})
    labels = ["O"] + [f"{prefix}-{aspect}" for aspect in aspects for prefix in ("B", "I")]
    train_aspects = {asp for asp in train_df["Aspect Name"].unique() if asp}
    if "U" in train_aspects:
        for prefix in ("B", "I"):
            tag = f"{prefix}-U"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.2,
        help="Ratio of data to reserve as held-out set (0.0-0.5).",
    )
...
def main() -> None:
    ...
    if not args.data_path.exists():
        raise FileNotFoundError(f"Tagged data not found at {args.data_path}")

    df_tagged = convert_tagged_to_aspect(str(args.data_path))
    category_to_valid_aspects = extract_category_to_valid_aspects(args.data_path)

    HELDOUT_SEED = 42
    heldout_dir = args.data_path.parent
    heldout_path = heldout_dir / f"heldout_set_ratio{args.heldout_ratio:.2f}.tsv"
    df_heldout: pd.DataFrame | None = None
    ...
    if args.heldout_ratio > 0:
        if heldout_path.exists():
            ...
            df_with_folds, df_heldout_generated = stratified_kfold_split_with_heldout(
                df_tagged,
            ...
            if existing_heldout_ids == generated_heldout_ids:
                ...
                error_msg = (
                    "ERROR: Existing held-out set does NOT match expected split!\n"
            ...
            df_with_folds, df_heldout = stratified_kfold_split_with_heldout(
                df_tagged,
                n_splits=args.num_folds,
                random_state=args.seed,
                heldout_ratio=args.heldout_ratio,
    ...
    label_list, label2id, id2label = build_label_resources(df_tagged, train_df)
    ...
    if df_heldout is not None:
        heldout_ids = set(df_heldout["Record Number"].unique())
        overlap_train = set(train_df["Record Number"].unique()).intersection(heldout_ids)
        overlap_valid = set(valid_df["Record Number"].unique()).intersection(heldout_ids)
        if overlap_train or overlap_valid:
            raise ValueError(
                "Held-out split overlaps with train/validation data. "
                f"Overlap in train: {len(overlap_train)}, overlap in validation: {len(overlap_valid)}."
            )
        print(f"Held-out records: {len(heldout_ids)} (no overlap with train/validation folds)")
