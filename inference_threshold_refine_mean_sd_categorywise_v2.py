#!/usr/bin/env python
"""Fast aspect-wise threshold optimization using percentile-based grid search, category-wise."""
...
from eval_score_fixed import calculate_fbeta, compute_competition_score
...
# ============================================================================
# FAST THRESHOLD SWEEP MODULE - Minimal, fast, well-commented
# ============================================================================

def _bio_spans_for_aspect(labels_str, token_idxs, offsets, text, aspect):
    """Extract GT spans for one aspect from BIO labels already filtered to train-tokens."""
    spans = []
    i = 0
    while i < len(labels_str):
        lab = labels_str[i]
        if lab == f"B-{aspect}":
            start_pos = token_idxs[i]
            j = i + 1
            while j < len(labels_str) and labels_str[j] == f"I-{aspect}":
                j += 1
            end_pos = token_idxs[j - 1]
            start_char, _ = offsets[start_pos]
            _, end_char = offsets[end_pos]
            spans.append(text[start_char:end_char].strip())
            i = j
        else:
            i += 1
    return spans  # list[str]


def _pred_spans_from_threshold(prob_vec, token_idxs, offsets, text, thr):
    """
    Convert a vector of per-token aspect probabilities (len = #kept tokens) into spans:
    take contiguous runs where prob >= thr and map to string spans via offsets.
    """
    keep = (prob_vec >= thr)
    if not np.any(keep):
        return []
    ...
    spans = []
    for g in groups:
        # g are local indices into kept tokens
        start_tok = token_idxs[g[0]]
        end_tok   = token_idxs[g[-1]]
        start_char, _ = offsets[start_tok]
        _, end_char   = offsets[end_tok]
        spans.append(text[start_char:end_char].strip())
    return spans  # list[str]


def _compact_percentile_grid(all_probs, max_candidates=15):
    """
    Build a small, high-tail-focused threshold grid from pooled probabilities.
    """
    if len(all_probs) == 0:
        return [0.0, 1.0]
    base = np.array([80, 90, 92, 94, 95, 96, 97, 98, 98.5, 99, 99.25, 99.5, 99.75, 99.9, 99.95])
    thr = np.quantile(np.asarray(all_probs), base / 100.0).clip(0.0, 1.0)
    thr = np.unique(thr)
    if len(thr) > max_candidates:
        # keep evenly spaced subset (always include ends)
        idx = np.linspace(0, len(thr) - 1, max_candidates).astype(int)
        thr = thr[idx]
    return thr.tolist()


def build_aspect_tables(samples, label2id_map, id2label_map, category_to_valid_aspects_map):
    """
    Organize samples by aspect for threshold sweeping.
    Store full sample info (not just per-aspect probs) so we can use argmax+BIO pipeline.
    """
    from collections import defaultdict
    per_cat_aspect = defaultdict(lambda: {"samples": []})

    for s in samples:
        cat = str(s["category"])
        valid_asps = category_to_valid_aspects_map.get(cat, set())

        # Extract GT spans per aspect for this sample
        text = s["text"]
        offs = s["offsets"]
        lbls = s["labels"]

        token_idxs = [i for i, l in enumerate(lbls) if l != -100]
        if not token_idxs:
            continue

        labels_str = [id2label_map[l] for l in (np.array(lbls)[token_idxs]).tolist()]

        # For each aspect, store GT spans
        for asp in valid_asps:
            gt_spans = _bio_spans_for_aspect(labels_str, token_idxs, offs, text, asp)

            per_cat_aspect[(cat, asp)]["samples"].append({
                "sample": s,  # Store full sample
                "gt_spans": gt_spans,
            })

    return per_cat_aspect


def sweep_best_threshold_for_category(category, per_cat_aspect, threshold_bins_cat, o_id, beta=0.2, max_candidates=15):
    """
    For a given category, sweep thresholds for each aspect using proper argmax+BIO pipeline.
    Start from mean threshold, try progressively higher thresholds (more conservative).
    """
    best_thr_per_aspect = {}
    fbeta_per_aspect    = {}

    # Find all aspects belonging to 'category'
    keys = [k for k in per_cat_aspect.keys() if k[0] == str(category)]

    for (cat, asp) in keys:
        bundle = per_cat_aspect[(cat, asp)]
        sample_records = bundle["samples"]

        if not sample_records:
            best_thr_per_aspect[asp] = 1.0
            fbeta_per_aspect[asp]    = 0.0
            continue

        # Build targets
        all_targets = []
        for rec in sample_records:
            rid = str(rec["sample"]["record_id"])
            for span in rec["gt_spans"]:
                all_targets.append({
                    "record_id": rid,
                    "category": cat,
                    "aspect_name": asp,
        ...
        # Get mean threshold for this aspect
        b_id = label2id.get(f"B-{asp}")
        i_id = label2id.get(f"I-{asp}")
        mean_threshold = 0.0
        if b_id is not None and b_id in threshold_bins_cat:
            mean_threshold = max(mean_threshold, threshold_bins_cat[b_id].get("mean", 0.0))
        if i_id is not None and i_id in threshold_bins_cat:
            mean_threshold = max(mean_threshold, threshold_bins_cat[i_id].get("mean", 0.0))

        # Build threshold grid using percentiles from high to low, stopping at mean
        # Get percentile bins for this aspect's B- and I- tags
        b_bins = threshold_bins_cat.get(b_id, {}).get("bins", np.array([])) if b_id is not None else np.array([])
        i_bins = threshold_bins_cat.get(i_id, {}).get("bins", np.array([])) if i_id is not None else np.array([])

        # Combine bins by taking max at each percentile (more conservative)
        if len(b_bins) > 0 and len(i_bins) > 0:
            combined_bins = np.maximum(b_bins, i_bins)
        elif len(b_bins) > 0:
            combined_bins = b_bins
        elif len(i_bins) > 0:
            combined_bins = i_bins
        else:
            # No statistics available, use conservative defaults
            combined_bins = np.array([mean_threshold, 1.0])

        # Build grid from high to low percentiles, stopping at mean
        # Use descending percentiles: 99.95, 99.9, ..., down to 50
        percentiles_desc = [99.95, 99.9, 99.75, 99.5, 99.25, 99, 98.5, 98, 97, 96, 95, 94, 92, 90, 85, 80, 70, 60, 50]
        grid = []
        for p in percentiles_desc:
            idx = int(p)  # bins array is indexed 0-100 for percentiles 0-100
            if idx < len(combined_bins):
                t = float(combined_bins[idx])
                if t < mean_threshold:
                    break  # Stop when below mean
                grid.append(t)

        # Always include mean as final candidate
        if mean_threshold not in grid:
            grid.append(float(mean_threshold))

        # Remove duplicates and sort descending (high to low)
        grid = sorted(list(set(grid)), reverse=True)

        # Limit to max_candidates
        if len(grid) > max_candidates:
            # Keep evenly spaced subset, always including first (highest) and last (mean)
            indices = np.linspace(0, len(grid) - 1, max_candidates, dtype=int)
            grid = [grid[i] for i in indices]

        best_f = -1.0
        best_t = mean_threshold

        # Sweep thresholds using PROPER argmax+BIO pipeline
        for t in grid:
            # Build threshold dict with this aspect's threshold
            test_thresholds = {o_id: 0.0}
            if b_id is not None:
                test_thresholds[b_id] = t
            if i_id is not None:
                test_thresholds[i_id] = t

            # Apply thresholds and extract spans for each sample
            all_preds = []
            for rec in sample_records:
                s = rec["sample"]
                masked_probs = torch.from_numpy(s["masked_probs"]).unsqueeze(0)
                base_preds = masked_probs.argmax(dim=-1)
                filtered = apply_thresholds(masked_probs, base_preds, test_thresholds, o_id).squeeze(0).numpy()
                ...
                for idx, l_id in enumerate(labels):
                    ...
                    token_idxs_list.append(idx)

                # Extract spans for this aspect only
                for extracted_asp, span in extract_spans(seq_p, token_idxs_list, s["offsets"], s["text"]):
                    if extracted_asp == asp:
                        all_preds.append({
            ...
            if not all_preds:
                ...
                comp_score = compute_competition_score(all_targets, all_preds, beta=beta)
                f = comp_score["per_category"].get(cat, 0.0)

            if f > best_f + 1e-12:
                best_f, best_t = f, t

        best_thr_per_aspect[asp] = float(best_t)
        fbeta_per_aspect[asp]    = float(best_f)

    return best_thr_per_aspect, fbeta_per_aspect


# ============================================================================
# END OF FAST THRESHOLD SWEEP MODULE
# ============================================================================


def fast_eval_from_per_cat_aspect(per_cat_aspect, th_cat1, th_cat2, label2id, beta=0.2):
    """
    Evaluate using the same per-aspect span construction used in the sweep.
    This ensures objective and evaluator are aligned.
    """
    all_targets, all_preds = [], []

    # Targets (extract once from all aspects)
    for (cat, asp), bundle in per_cat_aspect.items():
        for r in bundle["records"]:
            rid = str(r["record_id"])
            for span in r["gt_spans"]:
                all_targets.append({
                    "record_id": rid,
                    "category": cat,
                    "aspect_name": asp,
    ...
    # Predictions using aspect thresholds (same rule as sweep)
    for (cat, asp), bundle in per_cat_aspect.items():
        # Lookup the aspect's threshold from class thresholds
        b_id = label2id.get(f"B-{asp}", -1)
        i_id = label2id.get(f"I-{asp}", -1)
        if b_id == -1 and i_id == -1:
            continue

        thr_map = th_cat1 if str(cat) == "1" else th_cat2
        t = None
        if b_id in thr_map:
            t = thr_map[b_id]
        if t is None and i_id in thr_map:
            t = thr_map[i_id]
        if t is None:
            continue
        ...
        for r in bundle["records"]:
            rid = str(r["record_id"])
            spans = _pred_spans_from_threshold(r["prob_vec"], r["token_idxs"], r["offsets"], r["text"], t)
            for sp in spans:
                all_preds.append({
                    "record_id": rid,
                    "category": cat,
    ...
    comp_score = compute_competition_score(all_targets, all_preds, beta=beta)
    metrics = {"overall_score": comp_score["overall_score"]}
    metrics.update({f"cat_{cat}": score for cat, score in comp_score["per_category"].items()})
    return metrics
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fast aspect-wise threshold optimization using percentile-based grid search.")
    parser.add_argument(
        "--model_dir",
        type=Path,
        dest="model_dir",
    ...
    parser.add_argument("--max_passes", type=int, default=2, help="(Legacy parameter, not used in new approach)")
    parser.add_argument("--output_json", type=Path, default=Path("refined_thresholds_aspect_wise.json"), help="Where to save refined thresholds")
    parser.add_argument("--data_path", type=Path, default=Path("data/Tagged_Titles_Train.tsv"), help="Path to tagged training data")
    parser.add_argument(
        "--candidate_method",
        type=str,
        default="aspect_wise",
        choices=["mean_sigma", "percentile", "aspect_wise"],
        help="(Legacy parameter) Method is now aspect_wise by default",
    )
    parser.add_argument(
        "--max_candidates",
        type=int,
        default=15,
        help="Maximum number of threshold candidates per aspect (default: 15 for fast sweep)",
    )
...
def main() -> None:
    ...
    # Compute threshold statistics (used for baseline and reporting k-values)
    if args.threshold_bins and args.threshold_bins.exists():
        threshold_bins = np.load(args.threshold_bins, allow_pickle=True).item()
        print(f"Loaded threshold bins from {args.threshold_bins}")
    else:
        print("Computing threshold statistics...")
        _, threshold_bins = compute_threshold_statistics(samples, len(label_list))
        print("Threshold statistics computed.")
    ...
    # ========================================================================
    # USE FAST ASPECT-WISE THRESHOLD SWEEP (NEW APPROACH)
    # ========================================================================

    print(f"\n{'='*60}")
    print("Building aspect tables for fast threshold sweep...")
    print(f"{'='*60}\n")

    # Build aspect tables once (precompute prob vectors and GT spans)
    per_cat_aspect = build_aspect_tables(samples, label2id, id2label, category_to_valid_aspects)

    print(f"Built aspect tables for {len(per_cat_aspect)} (category, aspect) pairs")

    # Sweep best threshold for each category
    print(f"\n{'='*60}")
    print("Sweeping thresholds for Category 1 (using argmax+BIO pipeline)...")
    print(f"{'='*60}\n")
    best_thr_cat1, fbeta_cat1 = sweep_best_threshold_for_category(
        "1", per_cat_aspect, threshold_bins_cat1, o_id, beta=0.2, max_candidates=args.max_candidates
    )

    print(f"\n{'='*60}")
    print("Sweeping thresholds for Category 2 (using argmax+BIO pipeline)...")
    print(f"{'='*60}\n")
    best_thr_cat2, fbeta_cat2 = sweep_best_threshold_for_category(
        "2", per_cat_aspect, threshold_bins_cat2, o_id, beta=0.2, max_candidates=args.max_candidates
    )

    # Map aspect thresholds to class IDs (both B- and I- tags)
    refined_thresholds_cat1 = {o_id: 0.0}
    for asp, thr in best_thr_cat1.items():
        b_id = label2id.get(f"B-{asp}")
        i_id = label2id.get(f"I-{asp}")
        if b_id is not None:
            refined_thresholds_cat1[b_id] = thr
        if i_id is not None:
            refined_thresholds_cat1[i_id] = thr

    refined_thresholds_cat2 = {o_id: 0.0}
    for asp, thr in best_thr_cat2.items():
        b_id = label2id.get(f"B-{asp}")
        i_id = label2id.get(f"I-{asp}")
        if b_id is not None:
            refined_thresholds_cat2[b_id] = thr
        if i_id is not None:
            refined_thresholds_cat2[i_id] = thr

    # Calculate k values for reporting (how many steps from mean)
    k_values_cat1 = {}
    for cls, threshold in refined_thresholds_cat1.items():
        stats = threshold_bins_cat1.get(cls, {})
        mean = float(stats.get("mean", 0.0))
        std = float(stats.get("std", 0.0))
        if std > 1e-9:
            step = std * step_multiplier
            ...
            k_values_cat1[cls] = 0.0

    k_values_cat2 = {}
    for cls, threshold in refined_thresholds_cat2.items():
        stats = threshold_bins_cat2.get(cls, {})
        mean = float(stats.get("mean", 0.0))
        std = float(stats.get("std", 0.0))
        if std > 1e-9:
            step = std * step_multiplier
            ...
            k_values_cat2[cls] = 0.0

    # Final evaluation with both refined thresholds
    print(f"\n{'='*60}")
    print("Final evaluation with refined thresholds...")
    print(f"{'='*60}\n")
    ...
    print(f"\n{'='*60}")
    print("Final metrics (after fast aspect-wise threshold sweep):")
    print(f"{'='*60}")
    ...
    print(f"\n{'='*60}")
    print("Category 1 - Aspect-wise thresholds and F-beta scores:")
    print(f"{'='*60}")
    for asp in sorted(best_thr_cat1.keys()):
        thr = best_thr_cat1[asp]
        fbeta = fbeta_cat1[asp]
        print(f"  {asp:30s}: threshold={thr:.4f}, F-beta={fbeta:.4f}")

    print(f"\n{'='*60}")
    print("Category 2 - Aspect-wise thresholds and F-beta scores:")
    print(f"{'='*60}")
    for asp in sorted(best_thr_cat2.keys()):
        thr = best_thr_cat2[asp]
        fbeta = fbeta_cat2[asp]
        print(f"  {asp:30s}: threshold={thr:.4f}, F-beta={fbeta:.4f}")

    print(f"\n{'='*60}")
    print("Category 1 - Class-level view (threshold = mean + k * step_multiplier * std):")
    ...
    for cls_id in sorted(k_values_cat1.keys()):
        if cls_id in valid_ids_cat1:
            ...
            mean = float(stats.get("mean", 0.0))
            std = float(stats.get("std", 0.0))
            threshold = refined_thresholds_cat1.get(cls_id, 0.0)
            print(f"  {label:20s}: k={k:6.2f}  (mean={mean:.4f}, std={std:.4f}, threshold={threshold:.4f})")

    print(f"\n{'='*60}")
    print("Category 2 - Class-level view (threshold = mean + k * step_multiplier * std):")
    print(f"step_multiplier = {step_multiplier}")
    print(f"{'='*60}")
    for cls_id in sorted(k_values_cat2.keys()):
        if cls_id in valid_ids_cat2:
            ...
            mean = float(stats.get("mean", 0.0))
            std = float(stats.get("std", 0.0))
            threshold = refined_thresholds_cat2.get(cls_id, 0.0)
            print(f"  {label:20s}: k={k:6.2f}  (mean={mean:.4f}, std={std:.4f}, threshold={threshold:.4f})")
    ...
    print(f"  Refined (fast aspect-wise sweep): {final_metrics['overall_score']:.6f}")
    print(f"    Category 1: {final_metrics.get('cat_1', 0.0):.6f}")
    print(f"    Category 2: {final_metrics.get('cat_2', 0.0):.6f}")

    payload = {
        "method": "fast_aspect_wise_sweep",
        "category_1": {
            ...
            "aspect_thresholds": best_thr_cat1,
            "aspect_fbeta_scores": fbeta_cat1,
        },
        "category_2": {
            ...
            "aspect_thresholds": best_thr_cat2,
            "aspect_fbeta_scores": fbeta_cat2,
        },
        "candidate_method": "aspect_wise_percentile",
