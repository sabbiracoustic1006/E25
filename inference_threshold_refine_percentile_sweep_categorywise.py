#!/usr/bin/env python
"""Refine class-wise thresholds with percentile sweep search (100 → 50), category-wise."""
...
def build_candidate_thresholds_percentile_sweep(
    threshold_bins: Dict[int, Dict[str, object]],
) -> Dict[int, List[float]]:
    """Generate percentile-based candidates for each class.

    Starting from 1.0 (100th percentile), sweep down through percentiles (99, 98, 97, ...)
    continuing to lower percentiles until we drop below the mean. This creates a descending
    sequence of thresholds ending with the mean.
    """
    candidates: Dict[int, List[float]] = {}
    ...
    for cls, stats in threshold_bins.items():
        ...
        # Get the bins and percentiles arrays
        bins = stats.get("bins", np.array([]))
        percentiles = stats.get("percentiles", np.array([]))

        if len(bins) == 0 or len(percentiles) == 0:
            candidates[cls] = [float(np.clip(mean, 0.0, 1.0))]
            continue

        # Start with 1.0 (100th percentile)
        values = [1.0]

        # Sweep down from 99th percentile, continuing to lower percentiles
        # Stop when we drop below mean (don't include values below mean)
        for percentile_val in range(99, -1, -1):
            # Find the threshold value for this percentile
            # percentiles array goes from 0 to 100, we need to find the index
            idx = int(percentile_val)  # Since percentiles is linspace(0, 100, 101)
            if idx < len(bins):
                threshold = float(bins[idx])
            else:
                threshold = mean

            # Stop if we drop below mean (don't add this value)
            if threshold < mean:
                break

            values.append(threshold)

        # Add mean as the final threshold
        values.append(mean)

        # Deduplicate and sort in descending order for stable traversal
        clean = sorted({float(np.clip(v, 0.0, 1.0)) for v in values}, reverse=True)
...
def coordinate_descent_categorywise(
    ...
    print(f"Starting percentile sweep optimization for category {category}")
    print(f"{'='*60}\n")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output_json", type=Path, default=Path("refined_thresholds_percentile_sweep_categorywise.json"), help="Where to save refined thresholds")
...
def main() -> None:
    ...
    # Generate candidates using percentile sweep method
    print(f"\n{'='*60}")
    print(f"Using PERCENTILE SWEEP candidate generation (1.0 → percentile 99-50 → mean)")
    print(f"{'='*60}\n")
    candidate_thresholds_cat1 = build_candidate_thresholds_percentile_sweep(
        threshold_bins_cat1
    )
    candidate_thresholds_cat2 = build_candidate_thresholds_percentile_sweep(
        threshold_bins_cat2
    )
    ...
    print(f"\n{'='*60}")
    print("Final metrics (after category-wise percentile sweep):")
    print(f"{'='*60}")
    ...
    print(f"\n{'='*60}")
    print("Category 1 - Selected thresholds (percentile sweep method):")
    print(f"{'='*60}")
    ...
    print(f"\n{'='*60}")
    print("Category 2 - Selected thresholds (percentile sweep method):")
    print(f"{'='*60}")
    ...
    payload = {
        ...
        "candidate_method": "percentile_sweep",
        "step_multiplier": step_multiplier,
