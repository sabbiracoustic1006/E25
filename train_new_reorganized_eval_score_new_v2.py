...
def main() -> None:
    ...
    print("Training on FULL training dataset (no validation split)")

    # Load ALL relabeled data for training
    ...
    print(train_df.head())
    print("Unique Record Numbers in full training data:", len(train_df["Record Number"].unique()))
    ...
    # Create dummy validation set (required by Trainer but not used for model selection)
    # Using a small sample from training data
    valid_df = train_df.sample(n=min(100, len(train_df)), random_state=args.seed)
    ...
    print(f"Dummy validation set created with {len(valid_df)} records for monitoring")
    ...
    training_args = TrainingArguments(
        ...
        save_strategy="epoch",
        save_total_limit=8,  # Keep last 8 checkpoints (epochs 3-10)
        logging_steps=50,
        load_best_model_at_end=False,  # Don't load best model since we're not evaluating
        seed=args.seed,
        push_to_hub=False,
        label_smoothing_factor=args.label_smoothing,
        run_name=f"o_weight_{weight_tag}_full_train",
    )
    ...
    print(f"  Last {training_args.save_total_limit} checkpoints kept (epochs {args.epochs - training_args.save_total_limit + 1}-{args.epochs})")
    print(f"{'='*60}\n")
