#!/usr/bin/env python
"""Generate quiz submission TSV using category-wise optimised per-class thresholds."""
...
def load_categorywise_thresholds(
    threshold_path: Path, label2id: Dict[str, int]
) -> Tuple[Dict[int, float], Dict[int, float], Set[int], Set[int]]:
    """Load category-wise thresholds and valid class IDs from JSON."""
    ...
    # Load thresholds
    thresholds_cat1 = {
        label2id[label]: float(value)
        for label, value in cat1_data.get("thresholds", {}).items()
        if label in label2id
    }
    thresholds_cat2 = {
        label2id[label]: float(value)
        for label, value in cat2_data.get("thresholds", {}).items()
        if label in label2id
    }
    ...
    return thresholds_cat1, thresholds_cat2, valid_ids_cat1, valid_ids_cat2


def apply_thresholds_with_masking(
    probs: torch.Tensor,
    ...
    """Apply thresholds with category-specific class masking."""
    ...
    # Get predictions from masked probs
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create EvalAI submission using category-wise optimised thresholds.")
    parser.add_argument("--model_dir", type=Path, required=True, help="Directory with saved model")
    parser.add_argument("--threshold_path", type=Path, required=True, help="JSON file with category-wise thresholds")
    ...
    parser.add_argument("--output_file", type=Path, default=Path("tsvs/quiz_submission.tsv"), help="Output TSV path")
    ...
    # Load category-wise thresholds
    thresholds_cat1, thresholds_cat2, valid_ids_cat1, valid_ids_cat2 = load_categorywise_thresholds(
        args.threshold_path, label2id
    )
    o_id = label2id["O"]

    print(f"Loaded category-wise thresholds:")
    print(f"  Category 1: {len(thresholds_cat1)} thresholds, {len(valid_ids_cat1)} valid classes")
    print(f"  Category 2: {len(thresholds_cat2)} thresholds, {len(valid_ids_cat2)} valid classes")
    ...
    for start in tqdm(range(0, len(df_test), args.batch_size), desc="Generating"):
        ...
        with torch.no_grad():
            ...
            # Process each sample with its category-specific thresholds
            filtered_preds_list = []
            for i, category in enumerate(categories):
                sample_probs = probs[i:i+1]
                sample_base_preds = sample_probs.argmax(dim=-1)

                # Select thresholds and valid IDs based on category
                if category == "1":
                    thresholds = thresholds_cat1
                    valid_ids = valid_ids_cat1
                else:  # category == "2"
                    thresholds = thresholds_cat2
                    valid_ids = valid_ids_cat2

                filtered = apply_thresholds_with_masking(
                    sample_probs, sample_base_preds, thresholds, valid_ids, o_id
                )
