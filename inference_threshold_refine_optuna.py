#!/usr/bin/env python
"""Refine class-wise thresholds via Bayesian optimisation with Optuna."""
...
def optimise_thresholds_with_optuna(
    samples: List[Dict[str, object]],
    ...
    o_id: int,
    n_trials: int,
) -> Tuple[optuna.Study, Dict[int, float], float, Dict[str, float]]:
    """Run an Optuna study to maximise the competition score."""
    print(f"\n{'='*60}")
    print(f"Starting Optuna optimization (from Baseline 2: {baseline_2_score:.6f})")
    print(f"{'='*60}\n")

    def _suggest_threshold(trial: optuna.Trial, cls: int, default: float) -> float:
        stats = threshold_bins.get(cls, {})
        lower = float(stats.get("mean", default))
        lower = float(np.clip(lower, 0.0, 1.0))
        upper = 1.0
        if lower > upper:
            lower = upper
        return trial.suggest_float(f"class_{cls}", lower, upper)
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        for cls in thresholds:
            if cls == o_id:
                continue
            thresholds[cls] = _suggest_threshold(trial, cls, thresholds[cls])
        score, _ = evaluate_thresholds(samples, thresholds, o_id)
        return score
    ...
    for cls in best_thresholds:
        if cls == o_id:
            continue
        param_name = f"class_{cls}"
        if param_name in study.best_params:
            best_thresholds[cls] = float(study.best_params[param_name])

    best_score, best_metrics = evaluate_thresholds(samples, best_thresholds, o_id)
    return study, best_thresholds, best_score, best_metrics


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Refine thresholds via Optuna optimization.")
    parser.add_argument(
        "--model_dir",
        type=Path,
        dest="model_dir",
    ...
    parser.add_argument("--output_json", type=Path, default=Path("refined_thresholds_optuna.json"), help="Where to save refined thresholds")
...
def main() -> None:
    ...
    # Optuna optimization
    study, refined_thresholds, optimised_score, metrics = optimise_thresholds_with_optuna(
        samples,
        baseline_1_metrics,
        baseline_2_score,
        base_thresholds,
    ...
    print(f"\n{'='*60}")
    print("Optimized metrics (after Optuna):")
    print(f"{'='*60}")
    ...
    print(f"  Optimized (Optuna):            {optimised_score:.6f}")
    ...
    payload = {
        ...
        "optimized_score": optimised_score,
    }
