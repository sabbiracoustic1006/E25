#!/usr/bin/env python
"""Refine class-wise thresholds with mean-plus-sigma sweep search."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Refine thresholds with mean+σ sweep.")
    parser.add_argument(
        "--model_dir",
        type=Path,
        dest="model_dir",
    ...
    parser.add_argument("--output_json", type=Path, default=Path("refined_thresholds_mean_sd.json"), help="Where to save refined thresholds")
...
def main() -> None:
    ...
    print(f"\n{'='*60}")
    print("Refined metrics (after mean+σ sweep):")
    print(f"{'='*60}")
    ...
    print(f"  Refined (mean+σ sweep):        {metrics['overall_score']:.6f}")
