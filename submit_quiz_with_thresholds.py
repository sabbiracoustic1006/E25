#!/usr/bin/env python
"""Generate quiz submission TSV using optimised per-class thresholds."""
...
def load_thresholds(threshold_path: Path, label2id: Dict[str, int]) -> Dict[int, float]:
    with threshold_path.open("r", encoding="utf-8") as handle:
        payload = json.load(handle)
    saved_thresholds = payload.get("thresholds", {})
    return {label2id[label]: float(value) for label, value in saved_thresholds.items() if label in label2id}
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create EvalAI submission using optimised thresholds.")
    parser.add_argument("--model_dir", type=Path, required=True, help="Directory with saved model")
    parser.add_argument("--threshold_path", type=Path, required=True, help="JSON file from threshold_opt_eval_score.py")
    ...
    thresholds = load_thresholds(args.threshold_path, label2id)
    o_id = label2id["O"]
    ...
    for start in tqdm(range(0, len(df_test), args.batch_size), desc="Generating"):
        ...
        with torch.no_grad():
            ...
            filtered_preds = apply_thresholds(probs, base_preds, thresholds, o_id)
